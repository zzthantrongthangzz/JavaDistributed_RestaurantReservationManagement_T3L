...
public class CapNhatNhanVien_UI extends JPanel {
    ...
    private void khoiPhucNhanVien() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn khôi phục nhân viên " + txtHoTen.getText() + " làm việc lại?", "Xác nhận khôi phục", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
                ...
                protected Boolean doInBackground() throws Exception {
                    return nhanVienService.khoiPhucNhanVien(maNV);
                }
                @Override
                protected void done() {
    ...
    private void hienThiDialogNVDaNghi() {
        ...
        header.setDefaultRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                label.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 1, 1, MAU_LUOI_BANG), new EmptyBorder(10, 5, 10, 5)));
                return label;
            }
        });
        ...
        SwingWorker<List<NhanVien>, Void> loadWorker = new SwingWorker<List<NhanVien>, Void>() {
            ...
            protected List<NhanVien> doInBackground() throws Exception {
                if (nhanVienService == null) return new ArrayList<>();
                return nhanVienService.getAllNhanVienDaNghi();
            }
            @Override
            protected void done() {
                try {
    ...
    private JTextField taoTextFieldTimKiem(String holder) {
        ...
        txt.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(MAU_VIEN_THANH_TIM_KIEM, 1), new EmptyBorder(8, 15, 8, 40)));
        ...
        txt.addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                int iconX = txt.getWidth() - KICH_THUOC_ICON - 10;
                Rectangle iconBounds = new Rectangle(iconX, 0, KICH_THUOC_ICON + 10, txt.getHeight());
                txt.setCursor(iconBounds.contains(e.getPoint()) ? Cursor.getPredefinedCursor(Cursor.HAND_CURSOR) : Cursor.getPredefinedCursor(Cursor.TEXT_CURSOR));
            }
        });
    ...
    private JComboBox<String> taoComboBox(String[] items, Dimension size) {
        ...
        cmb.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(MAU_VIEN_THANH_TIM_KIEM, 1), new EmptyBorder(0, 15, 0, 5)));
    ...
    private JTextField taoFieldCoNhan(JPanel parent, String labelText, boolean enabled, String placeholder) {
        ...
        txt.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(MAU_VIEN_THANH_TIM_KIEM, 1), new EmptyBorder(8, 15, 8, 15)));
    ...
    private void loadDataToTable() {
        if (nhanVienService == null) return;
                ...
                try {
                    ...
                    for (NhanVien nv : danhSach) {
                        ...
                        tableModel.addRow(new Object[]{
                                nv.getMaNhanVien(), nv.getHoTen(), gioiTinh, nv.getSoDienThoai(),
                                nv.getEmail(), formatNgaySinh(nv.getNgaySinh()), nv.getDiaChi(),
        ...
        SwingWorker<List<NhanVien>, Void> searchWorker = new SwingWorker<List<NhanVien>, Void>() {
            ...
            protected List<NhanVien> doInBackground() throws Exception {
                if (keyword.toUpperCase().startsWith("NV")) {
                    return nhanVienService.timKiemNhanVienTheoMa(keyword);
                } else if (keyword.matches("\\d+")) {
                    return nhanVienService.timKiemNhanVienTheoSDT(keyword);
                } else {
                    return nhanVienService.timKiemNhanVienTheoTen(keyword);
                }
            }
            @Override
            protected void done() {
    ...
    private void capNhatNhanVien() {
        ...
        try {
            ...
            sqlNgaySinh = Date.valueOf(ngaySinhLD);
        ...
        SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
            @Override
            protected Boolean doInBackground() throws Exception {
                return nhanVienService.capNhatNhanVien(nvCapNhat);
            }
            @Override
            protected void done() {
                ...
                try {
                    if (get()) {
                        JOptionPane.showMessageDialog(CapNhatNhanVien_UI.this, "Cập nhật thông tin nhân viên thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                        loadDataToTable();
                    } else {
                        JOptionPane.showMessageDialog(CapNhatNhanVien_UI.this, "Cập nhật thất bại. Vui lòng kiểm tra lại dữ liệu!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    JOptionPane.showMessageDialog(CapNhatNhanVien_UI.this, "Mất kết nối với Server. Không thể thực hiện cập nhật!", "Lỗi Kết Nối", JOptionPane.ERROR_MESSAGE);
                }
            }
        };
        worker.execute();
    ...
    private void xoaNhanVien() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn 'Xóa' nhân viên này?", "Xác nhận xoá", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            SwingWorker<Boolean, Void> deleteWorker = new SwingWorker<Boolean, Void>() {
                @Override
                protected Boolean doInBackground() throws Exception {
                    if (nhanVienService == null) return false;
                    return nhanVienService.xoaNhanVien(maNV);
                }
                @Override
                protected void done() {
    ...
    private void thucHienSapXep(int loaiSapXep) {
        ...
        SwingWorker<List<NhanVien>, Void> sortWorker = new SwingWorker<List<NhanVien>, Void>() {
            ...
            protected List<NhanVien> doInBackground() throws Exception {
                if (nhanVienService == null) return new ArrayList<>();
                return nhanVienService.sapXepNhanVien(orderBy);
            }
            @Override
            protected void done() {
                try {
    ...
    private void thucHienLoc(String loaiLoc) {
        ...
        SwingWorker<List<NhanVien>, Void> filterWorker = new SwingWorker<List<NhanVien>, Void>() {
            ...
            protected List<NhanVien> doInBackground() throws Exception {
                if (nhanVienService == null) return new ArrayList<>();
                return nhanVienService.locTheoGioiTinh(gioiTinhValue);
            }
            @Override
            protected void done() {
                try {
    ...
    private void taiDuLieuChucVu() {
        SwingWorker<List<ChucVu>, Void> worker = new SwingWorker<List<ChucVu>, Void>() {
            ...
            protected List<ChucVu> doInBackground() throws Exception {
                if (chucVuService == null) return new ArrayList<>();
        ...
        btnThemDialog.addActionListener(e -> {
            ...
            SwingWorker<Object[], Void> worker = new SwingWorker<Object[], Void>() {
                ...
                protected Object[] doInBackground() throws Exception {
                    if (chucVuService == null) return new Object[]{false, "error", null};
                    ...
                    if (cvTonTai != null) {
                        ...
                        boolean ok = chucVuService.themChucVu(cvMoi);
