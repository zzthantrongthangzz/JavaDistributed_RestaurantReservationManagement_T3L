...
public class TraCuuNhanVien_UI extends JPanel {
    ...
    private void thucHienLoc(String loaiLoc) {
        ...
        SwingWorker<List<NhanVien>, Void> filterWorker = new SwingWorker<List<NhanVien>, Void>() {
            @Override
            protected List<NhanVien> doInBackground() throws Exception {
                return nhanVienService.locTheoGioiTinh(isNam);
            }
    ...
    private void timKiemNhanVien() {
        ...
        SwingWorker<List<NhanVien>, Void> worker = new SwingWorker<List<NhanVien>, Void>() {
            ...
            protected List<NhanVien> doInBackground() throws Exception {
                ...
                if (!tuKhoaMa.isEmpty()) {
                    ketQua = nhanVienService.timKiemNhanVienTheoMa(tuKhoaMa);
                }

                if (!tuKhoaTen.isEmpty()) {
                    List<NhanVien> listTheoTen = nhanVienService.timKiemNhanVienTheoTen(tuKhoaTen);
                    if (ketQua == null) {
                        ketQua = listTheoTen;
                    } else {
                ...
                if (!tuKhoaSDT.isEmpty()) {
                    List<NhanVien> listTheoSDT = nhanVienService.timKiemNhanVienTheoSDT(tuKhoaSDT);
                    if (ketQua == null) {
                        ketQua = listTheoSDT;
                    } else {
