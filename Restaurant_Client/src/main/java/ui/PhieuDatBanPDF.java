...
public class PhieuDatBanPDF {
    ...
    // Xuất phiếu đặt bàn ra file PDF (Đã áp dụng đa luồng SwingWorker)
    public static void xuatPhieuDatBanPDF(PhieuDatBan phieuDat, KhachHang khachHang, NhanVien nhanVien, List<BanAn> dsBanDat, double tienDatCoc) {
        ...
        // Bọc toàn bộ vào SwingWorker để tránh treo UI khi lặp truy vấn RMI
        SwingWorker<File, Void> worker = new SwingWorker<File, Void>() {
            @Override
            protected File doInBackground() throws Exception {
                // Khai báo kiểu Interface và Lookup từ RMI Registry
                IChiTietPhieuDatBan_DAO chiTietDAO = (IChiTietPhieuDatBan_DAO) Naming.lookup("rmi://localhost:1099/ChiTietPhieuDatBan_DAO");
                IMonAn_DAO monAnDAO = (IMonAn_DAO) Naming.lookup("rmi://localhost:1099/MonAn_DAO");
                ...
                if (dsChiTiet != null && !dsChiTiet.isEmpty()) {
                    ...
                    for (ChiTietPhieuDatBan ct : dsChiTiet) {
                        // Thao tác gọi Server RMI trong vòng lặp giờ đã an toàn vì nó chạy ngầm
            ...
            protected void done() {
                try {
                    ...
                    JOptionPane.showMessageDialog(null,
                            "Lỗi xuất PDF hoặc mất kết nối máy chủ:\n" + e.getMessage(),
